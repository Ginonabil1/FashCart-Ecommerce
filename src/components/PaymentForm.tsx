...
const PaymentForm = ({
  ...
  const { clearCart, cart, currentUser, addOrder } = useAppContext();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const handlePaymentForm: SubmitHandler<PaymentFormInputs> = (data) => {
    ...
    router.push(currentUser ? "/profile?tab=orders" : "/?checkout=success");
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <label htmlFor="cardHolder" className="text-xs font-medium text-gray-500">
          Name on card
        </label>
        <input
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="cardNumber" className="text-xs font-medium text-gray-500">
          Card Number
        </label>
        <input
          className="border-b border-gray-200 py-2 text-sm outline-none"
